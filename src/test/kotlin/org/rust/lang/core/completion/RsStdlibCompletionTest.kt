...
class RsStdlibCompletionTest : RsCompletionTestBase() {
    ...
    fun `test iter`() = @Suppress("DEPRECATION") checkSingleCompletion(
        "iter_mut()", """
        fn main() {
    ...
    fun `test derived trait method`() = @Suppress("DEPRECATION") checkSingleCompletion(
        "fmt", """
        #[derive(Debug)]
        struct Foo;
        fn bar(foo: Foo) {
    ...
    fun `test complete all in std in 'use' in crate root`() = checkContainsCompletion(
        "vec", """
        use std::/*caret*/;
    """
    )
