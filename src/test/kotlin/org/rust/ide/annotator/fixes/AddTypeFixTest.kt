...
class AddTypeFixTest : RsAnnotatorTestBase(RsSyntaxErrorsAnnotator::class) {
    ...
    fun `test static`() = checkFixByText(
        "Add type i32", """
    ...
    fun `test unknown type`() = checkFixByText(
        "Add type _", """
    ...
    fun `test partially unknown type`() = checkFixByText(
        "Add type (_, _)", """
    ...
    fun `test missing expr`() = checkFixIsUnavailable(
        "Add type", """
